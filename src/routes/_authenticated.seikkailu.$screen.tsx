...
function ScreenView() {
  ...
  // Track progress: bump profiles.current_screen to the max visited screen.
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        ...
        .eq("id", u.user.id)
        .maybeSingle();
      const cur = (data as { current_screen?: number } | null)?.current_screen ?? 1;
      if (n > cur) {
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-8">
        ...
        <StickyNote seed={`s${n}`}>
          ...
          <p className="text-muted-foreground">
            Tämä on paikanvaraaja. Vahvuusseikkailun näytön <strong>{n}</strong> ({world.title}) sisältö
            rakennetaan seuraavissa erissä.
          </p>
        </StickyNote>
      </div>
      <BottomNav n={n} saveState="idle" />
    </div>
  );
}